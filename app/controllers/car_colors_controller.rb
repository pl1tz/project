class CarColorsController < ApplicationController
  before_action :set_car_color, only: %i[ show update destroy ]
  ...
  def update
    if @car_color.update(car_color_params)
      if request.format.html?
        render file: "#{Rails.root}/public/index.html", layout: false
      else
  ...
  private
    ...
    def set_car_color
      @car_color = CarColor.find_by(id: params[:id])
      if @car_color.nil?
    ...
    def car_color_params
      params.require(:car_color).permit(:car_catalog_id, :background, :name, :image)
    end
end
